...
export const useEvent = (event: AppEvent) => {
  ...
  const oneMontFromNow = new Date();
  oneMontFromNow.setMonth(now.getMonth() + 1);

  const shouldShowCountdown = eventDate > now && eventDate < oneMontFromNow;
  ...
  return { host, isGoing, isHost, toggleAttendance: handleAttendanceToggle, cancelToggle: handleCancelToggle, shouldShowCountdown };
};
