...
import { type CollectionOptions, type Profile } from "../../lib/types";
  ...
  return (
    <div className="h-[50vh] overflow-auto">
      <table className="table">
        ...
        <tbody>
          {members?.map((member) => (
            <tr key={member.id} onClick={() => navigate(`/profiles/${member.id}`)} className="hover:bg-base-300 cursor-pointer">
              <td>
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col items-start gap-1">
                    ...
                    {followingIds.includes(member.id) && <div className="badge badge-soft badge-info badge-sm px-1 rounded-none">Following</div>}
                  </div>
                </div>
              </td>

              {followFilter === "all" && <td>{formatDateTime(member.createdAt)}</td>}

              <td>
                <button
                  onClick={(event) => followToggle(event, member)}
                  disabled={(loading && target === member.id) || member.id === currentUser?.uid}
