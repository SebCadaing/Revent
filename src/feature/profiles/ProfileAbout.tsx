    ...
    <>
      {!editMode ? (
        <div className="flex flex-col gap-3">
          ...
          {profile.description ? <div>{profile.description}</div> : <div>No description provided</div>}
        </div>
      ) : (
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4 w-full">
          <TextInput name="displayName" control={control} label="Display Name" rules={{ required: "Display Name is Required" }} />
          <TextArea name="description" control={control} label="Description" rows={4} />
          <button type="submit" disabled={!isDirty || !isValid || submitting} className="btn btn-primary self-end">
