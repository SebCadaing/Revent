...
export default function ProfileContent({ profile }: { profile: Profile }) {
  ...
  const items = [
    ...
    { key: "events", label: `Events`, description: `View events of ${profile.displayName}`, icon: ListBulletIcon },
    { key: "members", label: `Members`, description: `View other members of Revents`, icon: UserGroupIcon },
  ];
  const selected = items.find((items) => items.key === selectedItem) || items[0];
  const canEdit = currentUser?.uid === profile.id && (selectedItem === "about" || selectedItem === "photos");
  ...
  return (
    <div className="flex w-full gap-3 h-[60vh]">
