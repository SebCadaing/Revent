...
export default function EventDetailedSidebar({ event }: { event: AppEvent }) {
  ...
  return (
    ...
    from-primary to-black text-white"
      >
        {event?.attendees.length} People Coming
      </div>
      <div className="card-body">
        <div className="flex flex-col gap-3">
          {event?.attendees.map((attendee, index) => (
            <Link to={`/profiles/${attendee.id}`} key={attendee.id}>
              <div className="flex ga-3 align-middle justify-between items-center">
                <div className="flex gap-3 items-center ">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    {followingIds.includes(attendee.id) && <span className="badge badge-primary badge-soft">Following</span>}
                  </div>
                </div>
